...
public class BuildPage : PageWindow.PageElement
{
    /// <summary>
    /// 默认打包路径
    /// </summary>
    public const string RREFS_BUILD_OUT_PAHT = "RREFS_BUILD_OUT_PAHT";
    /// <summary>
    /// 默认打包平台
    /// </summary>
    public const string RREFS_BUILD_TARGET = "RREFS_BUILD_TARGET";
    /// <summary>
    /// C#包名
    /// </summary>
    ...
    /// 需要进行加密的AB包
    /// </summary>
    private static Dictionary<string, string> s_NeedByteEncryptionRecord = new Dictionary<string, string>();
    /// <summary>
    /// 使用字节加密
    /// </summary>
    private static bool s_UseByteEncryption;
    ...
    public override void OnGUI()
    {
        ...
        s_UseByteEncryption = EditorGUILayout.Toggle("使用字节加密", s_UseByteEncryption);
    ...
    private static void BuildDevelopment(LBuildParameter buildParameter)
    {
        ...
        CollectionFont(ref list, BuildPath.s_ConfigBuildPath);

        CollectionFont(ref list, BuildPath.s_ShaderBuildPath);

        CollectionAudio(ref list, BuildPath.s_SceneBuildPath);

        CollectionAudio(ref list, BuildPath.s_GUIBuildPath);
        ...
        //将新增/修改后的AB进行加密
        AssetBundleOffset(buildParameter, s_NeedByteEncryptionRecord);
    }
    ...
    private static void BuildCShareBytes(LBuildParameter buildParameter)
    {
        ...
        //打包update dll
        string hotfixDllSrcDir = SettingsUtil.GetHotUpdateDllsOutputDirByTarget(buildParameter.buildTarget);
        foreach (var dll in SettingsUtil.HotUpdateAssemblyFilesExcludePreserved)
        {
            ...
            if (File.Exists(dllPath))
            {
                ...
                deleteArray.Add(targetPath);
                GameLogger.INFO_FORMAT("HotUpdate Dll打包：{0}", dll);
            }
        }

        //打包aot dll
        string aotAssembliesSrcDir = SettingsUtil.GetAssembliesPostIl2CppStripDir(buildParameter.buildTarget);
        foreach (var dll in SettingsUtil.AOTAssemblyNames)
        {
            string dllPath = Path.Combine(aotAssembliesSrcDir, dll);
            ...
            ArrayUtility.Add<string>(ref abb.addressableNames, dll);
            deleteArray.Add(targetPath);
            GameLogger.INFO_FORMAT("AOT补充元数据 Dll打包：{0}", dll);
    ...
    public static AssetManifest_Bundle RefreshAssetsBundleManifest(string path)
    {
        ...
        foreach (var childDir in allDir)
        {
            ...
            foreach (var abName in allAB)
            {
                ...
                AssetBundle ab = s_UseByteEncryption ? AssetBundle.LoadFromFile(fullABPath, crc, 4) : AssetBundle.LoadFromFile(fullABPath, crc);
    ...
    public static void BuildVersion(LBuildParameter buildParameter)
    {
        ...
        BuildFiles buildingFile = GetBuildFiles();
        ...
        foreach (var file in assetManifest.bundleList)
        {
            if (file.fileFlag.HasFlag(AssetBundleInfo.AssetFileFlag.Build)
                || file.fileFlag.HasFlag(AssetBundleInfo.AssetFileFlag.Dll))
                buildingFile.Add(file);
        }

        string str = JsonHelper.ToJason(buildingFile, true);
    ...
    public static BuildFiles GetBuildFiles()
    {
        BuildFiles files = AssetDatabase.LoadAssetAtPath<BuildFiles>(BuildFiles.s_BuildFilesPath);

        if (files == null)
        {
            files = ScriptableObject.CreateInstance<BuildFiles>();
            AssetDatabase.CreateAsset(files, BuildFiles.s_BuildFilesPath);
        }
    ...
    public static void BuildBefore(LBuildParameter buildParameter)
    {
        if (!s_UseByteEncryption) return;
        //记录当前AB包
        s_NeedByteEncryptionRecord.Clear();
        AssetBundleRecord(buildParameter, ref s_NeedByteEncryptionRecord);
    }
