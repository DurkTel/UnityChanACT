...
namespace GAS.Editor
{
    ...
    {
        public class TimeLineAffectedBoxPreview : TimeLineClipPreview
        {
            private GameObject m_Shape;

            private FightBoxShape m_ShapeType;

            private AffectedEffectClip m_HitBoxClip;

            public Transform Avatar { get { return m_Preview.m_PreviewAvatar.transform; } }

            public TimeLineAffectedBoxPreview(TimeLineAbilityClip clip, TimeLinePreview preview) : base(clip, preview)
            {
                m_HitBoxClip = clip as AffectedEffectClip;
                OnCreatePrimitive();
            }
            ...
            private void OnCreatePrimitive()
            {
                if (m_ShapeType != m_HitBoxClip.boxShape || m_Shape == null)
                {
                    ...
                    if (m_Shape == null)
                        return;
                    m_Shape.GetComponent<Renderer>().sharedMaterial = m_Preview.WireframeMatG;
            ...
            private Vector3 GetScale()
            {
                ...
                {
                    case FightBoxShape.Box:
                        scale = m_HitBoxClip.boxParams.halfExtents;
                        break;
                    case FightBoxShape.Sphere:
